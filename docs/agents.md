...
• OpenRouter API integráció (AI elemzés)
• Open Food Facts API integráció (kalóriakövetés)
• Stripe integráció (webshop checkout)
• wger API adatimport (gyakorlat seed)
• refaktorálás
• technikai döntések
• kód minőség
...
• responsive design (desktop first + mobil)
...
/docs/backlog.md — Backend backlog (14 iteráció, 75 task)
/docs/frontend-backlog.md — Frontend backlog (11 iteráció, 77 task)
...
"Minden backlog tétel kész. A projekt backlog befejezve. Backend: 75/75 (100%), Frontend: 77/77 (100%), Összesített: 152/152 (100%)."
...
Projekt: FitTrack Pro — Edzés & Táplálkozás Webalkalmazás
...
Backend backlog: /docs/backlog.md (14 iteráció, 75 task)
...
Storage: Supabase Storage bucket-ek (avatars, exercises)
API integrációk:
  • wger REST API — gyakorlat-adatbázis statikus import (seed script, 150-200 gyakorlat)
  • OpenRouter API — heti AI fejlődéselemzés generálása (modell: nvidia/nemotron-3-super-120b-a12b:free)
  • Open Food Facts API — élelmiszer keresés és kalóriaadatok
  • Stripe — webshop checkout (test mode)
Fizetés: Stripe test mode (nem éles tranzakciók)
...
• Supabase kliens kezelés (server-side és client-side, @supabase/ssr)
• RLS policy-k írása és karbantartása
• Supabase Storage műveletek
• Supabase Auth integráció
• wger API seed script (gyakorlat-import)
• OpenRouter API integráció (AI elemzés endpoint)
• Open Food Facts API proxy (élelmiszer keresés)
• Stripe Checkout Session + Webhook handler
• Supabase RPC function-ök (dashboard aggregáció)
...
### Supabase
• Server-side kliens használata API route-okban (@supabase/ssr, createServerClient)
• Client-side kliens böngésző komponensekhez (src/lib/supabase/client.ts)
• RLS policy-k: minden tábla legyen védett
• Storage: megfelelő bucket-be feltöltés, URL generálás
• Auth: session ellenőrzés a middleware-ben és az API route-okban

### Külső API-k
• OpenRouter: szerver oldali proxy (/api/analysis), API key SOHA nem kerül a kliensre
• Open Food Facts: szerver oldali proxy (/api/food-search), válasz normalizálás
• Stripe: Checkout Session létrehozás (/api/checkout), Webhook handler (/api/webhooks/stripe)
• wger: egyszeri seed script (scripts/seed-exercises.ts), nem runtime API hívás

### Típusok
• Minden Supabase tábla típusa legyen definiálva src/types/database.ts-ben (Supabase CLI generálás: npx supabase gen types typescript)
...
Frontend backlog: /docs/frontend-backlog.md (11 iteráció, 77 task)
Nyelv: Magyar felhasználói felület, angol kód (változónevek, kommentek angolul)

Design filozófia: "SpaceX-inspirált sötét, ipari-minimalista" — kevés dekoráció, cinematikus felületek, ghost gombok, uppercase tipográfia
• Fő színek: #111111 (háttér), #780000 (sötétvörös akcent), #404040 (szürke felületek)
• Dark/Light mode toggle-lel (next-themes)
• Widgetes/kártyás dashboard grid
• Desktop first, de responsive

Animációs stack:
• Framer Motion — React komponens animációk (page transitions, staggered reveals, whileInView)
• CSS transitions/animations — mindennapi mikro-interakciók (hover, state changes)
...
### Színek — SpaceX-inspirált sötét, ipari-minimalista esztétika
Dark mode:
• Háttér: #111111 (közel fekete)
• Másodlagos háttér: #1a1a1a
• Felületi szín (kártyák): #1a1a1a / rgba(64,64,64,0.3) (üveghatás)
• Szöveg: #e0e0e0 (fő), #a0a0a0 (másodlagos)
• Elsődleges akcentszín: #780000 (sötétvörös) — CTA-k, aktív állapotok, kiemelések
• Szürke felületek: #404040 — szegélyek, másodlagos gombok, badge-ek
• Ghost gomb háttér: rgba(120, 0, 0, 0.1) — sötétvörös, 10% opacity
• Ghost gomb szegély: rgba(120, 0, 0, 0.35) — sötétvörös, 35% opacity

Light mode:
• Háttér: #ffffff, #f5f5f5
• Felületi szín (kártyák): white + enyhe shadow
• Szöveg: #1a1a1a (fő), #555555 (másodlagos)
• Akcentszín: #780000 (változatlan)
• Szürke felületek: #e0e0e0

Státusz színek: siker: emerald-500 | figyelmeztetés: amber-500 | hiba: red-500
...
### Tipográfia
• Display font (headingek): Barlow Condensed — Bold/ExtraBold, uppercase, letter-spacing: 0.96–1.17px
• Body font: Barlow — Regular/Medium, normál eset, jó olvashatóság
• Google Fonts / next/font/google betöltés optimalizálással
• A Barlow család a DIN ipari stílus legközelebbi Google Fonts megfelelője — geometrikus, mérnöki karakter

### Ikonok
• Lucide React — shadcn/ui natív ikon könyvtár, konzisztens 1.5px vonalvastagság
• Mobil tab bar ikonok: LayoutDashboard, Dumbbell, Flame, ShoppingBag, TrendingUp

### Navigáció
Desktop: felső navbar (logo + navigációs linkek uppercase Barlow Condensed letter-spacing-gel + profil avatar + theme toggle + kijelentkezés)
Mobil: felső sáv (logo + profil) + alsó tab bar (Dashboard, Edzés, Kalória, Shop, Fejlődés — 5 Lucide React ikon+label: LayoutDashboard, Dumbbell, Flame, ShoppingBag, TrendingUp), fix pozíció, safe area kezelés
Aktív állapot: sötétvörös (#780000) underline/highlight

### Kulcs UI minták
• shadcn/ui komponensek (Button, Card, Input, Dialog, Sheet, Tabs, Badge, Tooltip, Skeleton, Toast)
• Recharts grafikonok (vonaldiagram, oszlopdiagram, progress ring)
• dnd-kit drag & drop (edzéstervező — desktopra)
• Zustand globális state (kosár)
...
A skill fő elvei:
• Kódolás előtt határozz meg egy MERÉSZ esztétikai irányt — a mi esetünkben ez a "SpaceX-inspirált sötét, ipari-minimalista, sötétvörös/szürke akcenttel, ghost gombok és uppercase Barlow Condensed tipográfia" design filozófia
• Kerüld a generikus "AI slop" esztétikát: NEM használunk Inter/Roboto/Arial fontokat, NEM használunk sablonos lila gradienst fehér háttéren
• A tipográfia legyen egyedi és karakteres — Barlow Condensed uppercase letter-spacing-gel
...
• Spatial composition: átgondolt grid, konzisztens spacing, bőkezű negatív tér
...
A backend developer implementálja az API endpoint-okat és a Supabase RPC function-öket.
...
• ne implementálj backend logikát (API route-okat, RLS policy-kat) — az a backend developer dolga
...
A kód legyen:
• vizuálisan kifinomult (a SpaceX-inspirált sötét, ipari-minimalista design rendszernek megfelelő)
...
• shadcn/ui komponensek használata ahol lehetséges
• Képek: next/image komponens optimalizálással
• Fontok: next/font/google betöltéssel (Barlow Condensed + Barlow)
...
• CSS custom property-k használata a téma színekhez (globals.css)
• NEM localStorage a téma kivételével (Supabase a tartós állapotokhoz, Zustand a session-szintű state-hez)
...
2. Ha React életciklushoz kötött → Framer Motion. Megjelenés/eltűnés, viewport-ba érés (whileInView), layout váltás, page transitions.

Szabályok:
• Maximum 4-5 animáció oldalanként
• Minden animáció legyen kikapcsolható prefers-reduced-motion-nel
• Landing page: parallax + staggered reveals + count-up effektek (Framer Motion)
• Dashboard: skeleton loading + subtle fade-in kártyák
• Edzéstervező: drag & drop vizuális feedback (dnd-kit DragOverlay)
• Webshop: hover scale + kosár gomb "pattanás" animáció
...
1. **Másold a backlog fájlokat** a projekt `/docs/` mappájába:
   - `BACKEND_BACKLOG.md` → `/docs/backlog.md`
   - `FRONTEND_BACKLOG.md` → `/docs/frontend-backlog.md`
...
|---|---|
| F1 Design System & Layout Shell | Backend It. 1 (Projekt alapok) |
| F2 Auth oldalak | Backend It. 4 (Auth) |
| F3 Onboarding & Profil | Backend It. 5 (Profil CRUD) |
| F4 Dashboard | Backend It. 10 (Dashboard aggregáció) |
| F5 Gyakorlat böngésző | Backend It. 6 (Gyakorlat seed) + It. 8 (Keresés/szűrő) |
| F6 Drag & Drop edzéstervező | Backend It. 7 (Edzésterv CRUD) |
| F7 Kalóriakövetés | Backend It. 9 (Kalória) |
| F8 Fejlődéskövetés & AI elemzés | Backend It. 11 (Testsúly) + It. 12 (AI elemzés) |
| F9 Webshop | Backend It. 13 (Webshop + Stripe) |
| F10 Landing Page | Nincs közvetlen (statikus) |
| F11 Polish & Responsive Audit | Backend It. 14 (Integráció) |
